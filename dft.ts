...
export const fft = (xArr: Complex[]) => {
    ...
    for (let i = 0; i < N; i++) {
        if (i % 2 === 0) even.push(xArr[i]);
        else odd.push(xArr[i]);
    }
...
export const makeSample = (from: number, interval: number, to: number, func: (x: number) => number) => {
    const sample: number[] = [];
