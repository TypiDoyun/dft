import "./dft.js"
import { dft, fft, makeSample } from "./dft.js";
...
let isHolding = false;
const N = 100;
const samples: number[] = new Array(N).fill(-1);
...
canvas.addEventListener("mouseup", eventData => {
    ...
    const frequencies = fft(
        ...
        sample.map(element => new Complex((element / canvas.height - 0.5) * 2))
    ).map(frequency => frequency).slice(0, 100)
